...
# REV5 Addition — Passkey-Only Accounts, Activation PIN, Pending Access, and First-Run Profile

This revision changes the account/authentication flow.

The existing working MiniMax integration, typing engine, progress system, AI Training Console, save behavior, shortcuts, and admin/test functionality must remain intact.

The app now uses:

1. **Passkey = identity/authentication**
2. **Activation PIN = one-time authorization to enter the application**
3. **Parent/admin PIN = privileged administrative/test controls only, if the existing implementation still uses it**

Do **not** use a user PIN as a normal login credential after this revision.

A returning activated learner signs in with a **passkey only**.

## Architecture decision

Do not use Cloudflare Access as the learner account system.

Cloudflare Access can protect an application and can enforce WebAuthn-backed MFA, but Cadence needs its own self-service account creation, application-owned user records, passkey registration, activation state, learner profile, character, curriculum progress, and account lifecycle.

Implement application-level WebAuthn/passkeys.

### Recommended Cloudflare layout

Keep the existing Python/FastAPI Worker for the typing application and MiniMax integration.

Add a small dedicated **TypeScript authentication Worker** using:
...
Use a Cloudflare **Service Binding** between the application Worker and auth Worker instead of making public Worker-to-Worker HTTP calls.
...
Browser / React
      ...
      | same-origin HTTPS
      v
Cadence application Worker / router
      |
      +---- /api/auth/* ----------------------+
      |                                      |
      ...
      |                           D1 auth tables
      |
      +---- protected app/API --------------> Python/FastAPI Worker
                                              typing + MiniMax
```

The browser must see one application origin.

Do not move MiniMax into the authentication Worker.

Do not expose the MiniMax API key or authentication secrets to the browser.

## WebAuthn relying-party configuration

Use production values tied to the actual Cadence hostname.

Example only:

```text
APP_ORIGIN=https://cadence.example.com
WEBAUTHN_RP_ID=cadence.example.com
WEBAUTHN_RP_NAME=Cadence
```

Do not casually change the RP ID after users have registered passkeys. Existing credentials are scoped to the relying party.

Keep localhost development configuration separate from production.

## Required passkey behavior

Cadence is now **passwordless**.

There is no:
- password login,
- email/password login,
- PIN login for normal users,
- secret recovery question.

The normal login screen contains one primary action:

```text
Sign in with passkey
```

Account creation contains:

```text
Create account
```

Creating an account immediately leads into passkey registration.

Use discoverable WebAuthn credentials so returning users do not need to type a username before authenticating.

Recommended registration configuration:
...
for normal consumer passkey registration unless a later requirement explicitly needs attestation.

The verification endpoint must verify:
- expected challenge,
- expected origin,
- expected RP ID,
- WebAuthn signature,
- credential state/counter as required by the library.

Save credential transports returned by the client.

Allow platform authenticators such as Windows Hello, Apple Touch ID / Face ID, Android device authentication, and compatible roaming/security-key authenticators.

Do not require a specific authenticator brand.

## Passkey-only account creation without profile data first

The learner enters their name, age, school status, and grade **after activation**.

Therefore registration must not require their real name first.

When a new account is created, generate:

```text
user_id = cryptographically random UUID/ID
webauthn_user_id = stable random byte identifier
temporary_handle = generated internal account label
```

Example internal handle:

```text
cadence-7K4M9P
```

Use it as the initial WebAuthn username/display label if required by the registration ceremony.

After activation and onboarding, store the learner's chosen display name separately.

Do not change the stable WebAuthn user ID when their display name changes.

Where supported, send the WebAuthn current-user-details signal after a display-name change so credential managers can show current account information.

## Optional anti-bot protection for account creation

Protect **new account creation**, not every typing session, with Cloudflare Turnstile.
...
The Turnstile token must be validated server-side before creating the pending account / registration ceremony.

Do not rely on the client-side widget alone.

## Authentication database additions

Create migrations. Do not manually mutate production schema.

Suggested `users` additions/table:
...
Never store activation PINs in plaintext.

## Challenge safety

Every WebAuthn ceremony must use a fresh random challenge.

Challenge records must:
- expire quickly,
- be tied to the appropriate session/user,
- be invalidated after verification,
- never be reusable.

A five-minute challenge lifetime is a reasonable default.

Do not put an unsigned challenge in localStorage and trust it during verification.

## Session cookie

After successful passkey authentication, issue an application session.

Use a secure opaque cookie such as:

```text
__Host-cadence_session
```
...
PENDING ACTIVATION
   |
   | correct one-time activation PIN
   v
ACTIVE + NEEDS ONBOARDING
   |
   | onboarding complete
   v
ACTIVE LEARNER
...
Passkey authentication
   |
   +---- pending_activation ---> activation waiting screen
   |
   +---- active + onboarding incomplete ---> onboarding
   |
   +---- active + onboarding complete ---> game main menu
   |
   +---- disabled ---> access-disabled screen
```

These states must be enforced server-side.

## Public landing screen

Keep the entry screen extremely simple and make it obvious that this is a typing-learning application.
...
A focused typing trainer that teaches touch typing,
tracks how you improve, and adapts practice as you go.
...
Do not show learner profiles, character lists, progress, WPM, or AI controls publicly.

## Revision to the earlier character-before-login flow

Change the earlier instruction that put character selection before login.

With passkey-only authentication, **do not publicly display user profiles or characters before authentication**.
...
Public landing
→ passkey login
→ activation gate if needed
→ first-run learner setup
→ character selection
→ game-style main menu
→ training
```

For returning active users:

```text
Public landing
→ Sign in with passkey
→ game-style main menu with their character
```

This avoids leaking who uses the application and fits discoverable passkey authentication better.
...
Create your Cadence account with a passkey.
No password to remember.

Your device may use Face ID, Touch ID,
Windows Hello, Android screen lock, or a security key.
...
Do not ask for name, age, school, grade, or typing skill yet.
...
Then move immediately to activation state.

## Activation PIN is authorization, not authentication

A valid passkey proves **which account this is**.

The activation PIN determines **whether that account is allowed into Cadence**.

Protected typing/AI/progress endpoints must require:
...
A pending account may only access a small allowlist such as:
...
Pending accounts must not access:
- MiniMax,
- training content,
- saved learner sessions,
- character/profile data beyond minimal pending status,
- admin APIs.

## Activation PIN design

Activation PINs should be:
- created by parent/admin,
- one-time use,
- account-bound,
- hashed,
- invalid after successful activation,
...
```text
8-digit numeric activation PIN
```
...
Do not hard-code a universal activation PIN.

Do not use the admin PIN as the learner activation PIN.

## Admin activation workflow

Add a pending-player section in admin mode:
...
The admin can give the PIN to the learner outside the app.

## Pending / locked application screen

If a user signs in successfully with a passkey but is not activated, do not show the normal app.

Use a friendly holding screen with the requested sad face:
...
Your account is set up and your passkey works.
Cadence is just waiting for an activation PIN.

Once you get your PIN, come back here and you're ready to train.
...
```text
Access: Waiting for activation
```

Do not use punitive copy such as:
- You failed
- Unauthorized user
- Permission denied
- Account rejected

unless the account is actually disabled.
...
Got your access PIN? Drop it here.
...
Not yet?
No stress — come back when you have it.
```
...
Do not reveal whether another account owns the PIN.

Rate-limit this route aggressively.

Recommended:
- maximum 5 failed attempts per account in a short window,
- IP-level abuse protection as a second layer,
- increasing cooldown after repeated failures.
...
## First-run onboarding after activation

Only now collect learner profile details.

The UI should feel game-like/current but not forced.
...
This is what your coach will use.
```

Store as `display_name`.

Do not require a legal/full name.

### Step 2 — Age

Use playful but neutral copy:
...
```text
How many laps around the sun? ☀️
```

Do not show age publicly.

Do not send age to MiniMax by default.
...
Show only if school status is `student`.
...
Then run the character-selection flow:

```text
🧩 PICK YOUR TRAINING STYLE

Choose your Cadence character.
...
Finish onboarding simply:

```text
READY, {display_name}?

Cadence teaches touch typing one skill at a time.
You'll learn where your fingers go, build clean rhythm,
and get faster as the movement becomes automatic.

No need to chase speed yet.

[ Start training ]
```

Then enter the existing orientation/home-row flow.
...
What level are you on?
Pick your training style.
Want a clean run or something harder?
Locked in?
```
...
```text
Yo fam!
You're literally cooking fr fr!
Sigma typing mode!
```

The product voice remains calm, smart, modern, and teen-appropriate.

## Returning activated user flow
...
After passkey verification, route directly to the authenticated game menu.
...
Passkeys
- Windows Hello · This PC
- iPhone passkey

[ Add another passkey ]
```

Recommend a second passkey to reduce lockout risk.

Normal login remains passkey-only.

## Recovery

Because there is no password fallback, define recovery explicitly.

V1 recommendation:
- admin-assisted recovery,
- admin verifies the learner outside the app,
- admin places the account into controlled passkey-recovery state,
- learner registers a new passkey,
- progress remains attached to the same user ID.

Do not use security questions.

Do not create a new learner profile just because a passkey is lost.
...
POST /api/admin/users/:id/activation-pin
POST /api/admin/users/:id/disable
POST /api/admin/users/:id/recovery/start
```
...
Do not allow onboarding writes while the account is still pending.

## Protected API middleware

The Python application must never trust a user ID supplied by the browser.

Resolve the authenticated user from the secure session.
...
request
→ session cookie
→ Auth Worker verifies session
→ internal authenticated principal
→ Python route checks account_status
→ route executes
```

MiniMax routes require:
...
Pending accounts must never consume MiniMax usage.

## Security requirements

Serve production only over HTTPS.

Keep `expectedOrigin` and RP ID explicit.

Do not trust a request-supplied origin.

Do not allow arbitrary iframe origins to perform passkey ceremonies.

Apply dedicated rate limits to:
...
- session tokens,
- MiniMax key,
- full credential/public-key dumps.

## Learner privacy

Name, age, school status, and grade are not required for normal MiniMax lesson generation.

Do not include them in MiniMax prompts by default.

AI prompts should primarily receive:
- curriculum stage,
- unlocked keys,
- weak keys,
- aggregate typing performance,
- chosen topic,
- desired drill length/difficulty.

If an age band is genuinely useful for content, derive a coarse band server-side rather than sending the full learner profile.

Do not send the learner's name simply to personalize coach text.

## Under-13 implementation guardrail

Because the application asks for age/grade, it may establish that a user is under 13.

Before any broad/public rollout, add a child-privacy/compliance review for the intended audience and jurisdiction.

For engineering now:
- do not collect school name,
- do not collect learner contact details,
- do not expose profiles publicly,
- do not send profile identity data to MiniMax,
- support parent/admin deletion/export,
- minimize retained personal data.

## UI route map after REV5
...
/register
  Passkey explanation → Turnstile → WebAuthn creation

/login
  Passkey only

/activate
  Pending activation / PIN redemption

/onboarding
  Name → age → school → grade → character → ready

/menu
  Authenticated game-style menu
...
/settings
  Character, display name, accessibility, coach, passkeys

/admin
  Protected parent/admin management
```

## Migration from the current PIN implementation

The current application already has PIN functionality.

Classify each PIN use first:

```text
CURRENT PIN PURPOSE          REV5 RESULT
----------------------------------------------------------
normal learner login        replace with passkey
site-wide learner gate      replace with account activation
parent/admin controls       keep
test/developer controls     keep if useful
```

For existing approved learners:
- preserve their existing user ID,
- preserve all progress/history,
- mark them active,
- require one-time passkey enrollment on next entry,
- retire their normal learner login PIN after passkey enrollment.

Keep parent/admin PIN controls separate.

## Required tests

Add tests for at least:

1. Public page says Cadence teaches typing.
2. Login offers passkey only.
3. Registration does not collect real profile data before passkey creation.
4. Valid registration creates a pending account.
5. Pending users cannot access training.
6. Pending users cannot access MiniMax.
7. Pending user sees friendly sad-face screen.
8. Wrong activation PIN fails.
9. Activation attempts are rate-limited.
10. Correct account-bound PIN activates exactly once.
11. Consumed PIN cannot be reused.
12. Active first-time user goes to onboarding.
13. Grade question appears only for school users.
14. Character selection occurs after authentication/activation.
15. Returning user signs in with passkey and reaches menu.
16. There is no normal password/PIN fallback.
17. Multiple passkeys can belong to one user.
18. Logout invalidates session.
19. WebAuthn challenges expire and cannot be replayed.
20. Wrong origin/RP ID fails verification.
21. Disabled account cannot enter even with valid passkey.
22. Direct frontend route navigation cannot bypass server authorization.
23. Profile name/age/grade are absent from normal MiniMax requests.
24. Existing progress survives auth migration/recovery.

## Definition of Done

REV5 is complete only when:

- passkey registration works on the Cloudflare-hosted production origin,
- passkey login works without username/password,
- WebAuthn verification is server-side,
- challenges cannot be replayed,
- pending users are blocked server-side,
- pending screen clearly says activation is still needed,
- one-time activation PIN works and is hashed/rate-limited,
- successful activation leads to onboarding,
- onboarding includes fun name/age/school/grade/character steps,
- character selection happens after authentication,
- active users land in the game-style menu,
- MiniMax remains blocked for pending accounts,
- existing progress survives migration,
- normal learner login has no password/PIN fallback,
- parent/admin controls remain protected.

---

## REV5 research references for Codex

Use current official documentation when implementing package/API details.
...
# REV6 — Persistent PIN Activation Version, 2-Day Login Lock, and Mandatory Finger-Guidance Animation

> **REV6 overrides conflicting activation-PIN behavior in REV5.**
>
> In particular, REV5's account-bound one-time activation PIN model is replaced by the versioned activation-gate model below.
>
> Keep the passkey architecture, Cloudflare auth Worker, D1 storage, MiniMax protection, onboarding, game menu, character system, and all other non-conflicting REV5 requirements.

## 1. Final authentication model

Cadence now has three separate concepts. Do not mix them.

### A. Passkey = login / identity

A learner must authenticate with their passkey to create a new authenticated application session.

The normal learner login has no:

- password,
- username + password form,
- normal learner login PIN,
- email login fallback.

The primary returning-user action remains:

```text
[ Sign in with passkey ]
```

Use discoverable WebAuthn credentials/passkeys.

### B. Activation PIN = app access approval

The activation PIN is **not** the user's login credential.

It is an administrator-controlled access gate that decides whether an authenticated account may enter the typing application.

After a user successfully enters the **current** activation PIN:

- save that approval on their account;
- do not ask them for the PIN on subsequent passkey logins;
- keep the approval until the administrator changes/rotates the activation PIN.

The learner still authenticates using a passkey whenever a new authenticated session is required.

### C. Parent/admin PIN = privileged controls

If the application already uses a separate parent/admin PIN for:
...
keep it separate from the learner activation PIN.

Never make the admin PIN identical to, or interchangeable with, the learner activation PIN.

---

# 2. Versioned activation gate

Implement the activation PIN using a **server-side activation version**.

Do not store a plaintext activation PIN and do not compare a PIN in the browser.

## Server configuration/state

Create an application-access configuration record such as:
...
Each learner records the latest activation version they have successfully accepted:

```sql
ALTER TABLE users
ADD COLUMN accepted_activation_version INTEGER;
```
...
```sql
ALTER TABLE users
ADD COLUMN activation_verified_at TEXT;
```

## Access check

After passkey authentication, compute:

```text
current_activation_version =
    app_access_config.activation_version

user_activation_version =
    users.accepted_activation_version
```

Then:

```text
if user_activation_version == current_activation_version:
    activation_gate_satisfied = true
else:
    activation_gate_satisfied = false
```

A learner may access training only when:

```text
valid authenticated passkey session
AND
account_status == active
AND
accepted_activation_version == current activation_version
```

This rule must be enforced on the server, not only in React.

---

# 3. New-user activation

A new account created with a passkey initially has:

```text
account_status = active
accepted_activation_version = NULL
onboarding_completed = false
```

The learner has proved their account identity, but they have not yet been approved through the current Cadence access gate.

After passkey registration/login, send them to:

```text
/activate
```

Do not expose the training application yet.

## Activation UI

Use the friendly locked-state design:
...
Your passkey works, but Cadence is still locked.

You need the current access PIN before you can start training.
Once you have it, drop it in here and you're good to go.
...
Alternative concise copy is acceptable, but keep the emotional message:

```text
your account works;
the app is simply waiting for access approval.
```

Do not make the learner feel rejected.
...
Still waiting on it?
No stress. Your account and passkey are ready.
Come back when you have the PIN.
```

The existing numeric PIN length may be preserved if already deployed safely.

If designing it fresh, 8 numeric digits is preferred over a very short 4-digit internet-facing gate.
...
# 4. Persistent activation approval

After a correct PIN submission:

1. verify the PIN server-side;
2. set:
...
3. set `activation_verified_at = now`;
4. rotate or refresh the authenticated application session;
5. if onboarding is incomplete, route to `/onboarding`;
6. otherwise route to `/menu`.

The learner **does not enter the PIN again** on later logins while:

```text
accepted_activation_version == current activation_version
```

Only the passkey is needed for normal authentication.

---

# 5. What happens when the administrator changes the PIN

Changing the activation PIN must also increment `activation_version`.

Do this in one transaction/logical operation:
...
Do **not** individually rewrite every user's activation record.

The version mismatch automatically causes every learner to require the new PIN.

Example:

```text
Current gate version: 7
Soren accepted version: 7
→ access allowed

Admin changes PIN
Current gate version: 8
Soren accepted version: 7
→ PIN required
```

After Soren enters the new PIN successfully:

```text
Soren accepted version: 8
→ access allowed again
```

This is the required behavior.

## User experience after a PIN rotation

The user still begins with their passkey:

```text
[ Sign in with passkey ]
```

After successful passkey authentication, detect the activation-version mismatch.
...
Cadence has a new access PIN.

Your passkey is still good — you just need the new PIN
before jumping back into training.
...
Optional small sad-face variant:

```text
:(
Locked for now — but your progress is safe.
```

Do not say the user's account was deleted, rejected, or lost.

All saved typing progress remains intact.

---

# 6. Two-day authentication lock

The activation approval above persists until the activation PIN changes.

Separately, authenticated login sessions have a **hard maximum lifetime of 2 days**.

Set:

```text
AUTH_SESSION_MAX_AGE = 172800 seconds
```

or equivalent server-side expiry:
...
A passkey-authenticated session can never remain accepted past its server-side two-day expiration.
...
application locks
→ return to login
→ user signs in with passkey again
```

After the new passkey login:

```text
if accepted_activation_version == current activation_version:
    go directly to menu
else:
    ask for current activation PIN
```

The 2-day timeout must **not** cause a correctly activated learner to re-enter an unchanged PIN.
...
ACTIVATION PIN APPROVAL:
persists until activation PIN/version changes
```

## Explicit logout / Close App

Explicit Sign Out must invalidate the server session immediately.

If the existing `Close App` command is intended to log the learner out, it should:

```text
save current training state
→ invalidate auth session
→ clear auth cookie
→ show safe-to-close screen
```

The next entry requires the passkey again.

If the product decides that closing the browser without using `Close App` may retain a session, the server-side 48-hour hard limit still applies.

Do not depend solely on the browser cookie expiration; validate the session's server-side `expires_at` on every protected request.

---

# 7. Passkey implementation requirements confirmed by current documentation

Use current SimpleWebAuthn APIs and adapt syntax to the installed package version.

## Registration

Generate a discoverable credential:
...
unless there is a later explicit reason to collect attestation.
...
- counter,
- WebAuthn user ID,
- credential device type if available,
- backup state if available,
- transports.

The browser/authenticator keeps the private key. Cadence never receives or stores it.

## Usernameless/passkey login

Generate authentication options that allow discoverable credentials.
...
The returned credential ID/user handle identifies the account.

Do not require the user to type a username first.

## Challenges

Store the server-generated WebAuthn challenge against a temporary session identifier.

Recommended challenge lifetime:

```text
5 minutes
```

After an authentication/registration verification attempt:

```text
delete/invalidate challenge
```

even when practical after failure, so it cannot be replayed.

## Cloudflare placement

Keep the small WebAuthn implementation in the TypeScript Auth Worker.

Use Cloudflare Service Bindings for internal Worker-to-Worker calls instead of sending authentication validation through a public hostname.
...
- activation audit metadata,
- temporary challenges.

Keep MiniMax in the existing backend and require a fully authorized principal before an AI endpoint can run.

---

# 8. Updated login/activation state machine
...
# 9. Onboarding after activation

Preserve the REV5 onboarding but keep it short and game-like.
...
Do not ask for profile data before the learner has passed the access gate.
...
The final screen should explain the app in very few words.
...
Cadence is here to teach you touch typing.

You'll learn where each finger goes,
build smooth rhythm, and get faster naturally.

Technique first. Speed follows.

[ Start training ]
```

This should not turn into a marketing page.

---

# 10. MANDATORY: keyboard hand/finger visualization

This is a **core V1 teaching feature**, not optional polish and not a future backlog item.

It comes directly from the original typing product specification.

Codex must implement the actual animation.

Do not substitute:

- a static keyboard highlight only,
- text that says "use your index finger",
- an icon with no motion,
- a TODO,
- a placeholder box,
- an external animation package that does not map accurately to keys.

The app should visually teach:

1. where both hands start;
2. which finger owns each key;
3. which finger should move for the current target;
4. the path from home position to the target;
5. returning to home position.

---

# 11. Home-row visualization

Some curriculum stages must begin with a short hand-placement visualization before the typing round starts.

At minimum, the beginner orientation/home-row stage must show it.

Recommended tutorial screen:
...
Do not literally rely on those finger emojis if they look visually awkward or inappropriate in the production UI. They illustrate the concept only.

The real UI should use a clean hand diagram.
...
Thumbs rest naturally near the spacebar.

Highlight the physical bumps on:
...
They help your hands find home without looking down.
```

---

# 12. Finger ownership map

Create a canonical data structure rather than hard-coding ownership in animations.
...
- use the opposite hand's Shift key when practical;
- later curriculum should explicitly teach Shift mechanics.

The exact punctuation map may be adjusted for the selected US keyboard layout, but it must be centralized and tested.

---

# 13. Animated hand implementation

Use SVG/CSS/React animation or an equivalently lightweight native front-end implementation.
...
Do not require raster images for every movement.

## Visual hand layer

Create semi-transparent left and right hand shapes aligned above/below the visual keyboard.

Each hand should have independently addressable finger elements:
...
Each finger element must be highlightable and animatable.
...
When the target is `R`, for example:

1. visual keyboard highlights R subtly;
2. left index finger becomes slightly more visible;
3. left index moves from the F home position toward R;
4. finger pauses briefly above/touches R;
5. R key gives a small quiet response;
6. finger returns toward F;
7. guide fades.
...
The animation should communicate reach direction, not pretend to simulate real biomechanics perfectly.

---

# 14. When the animation must appear

Show finger guidance in these situations.

### Required trigger A — first introduction of a key

Before or at the start of a lesson introducing a new key:
...
→ animate reach
→ user begins micro-drill
```

### Required trigger B — home-row tutorial

At beginner onboarding/orientation:

```text
place both hands
→ highlight A S D F / J K L ;
→ emphasize F/J
→ optionally animate fingers settling into place
```

### Required trigger C — repeated error

Use the original product rule as guidance:

```text
learner repeatedly misses R
→ do not interrupt on first mistake
→ after repeated misses, show subtle R hint
→ if pattern continues, display finger movement
→ later insert R-focused mini-drill
```

A reasonable starting trigger:

```text
same target key missed 3 times in a rolling short interval
```

Make this threshold configurable.

### Required trigger D — user asks for help

Training Console or lesson UI should provide:
...
This replays the movement without penalizing the learner.

### Optional trigger E — Learning Mode

If the user enables a learning-assist setting:

```text
Finger guidance: More
```

the animation can appear more often.

Do not animate on every keystroke by default.

---

# 15. Animation should fade as mastery improves

The original specification says hand help should disappear once the learner demonstrates the movement successfully.

Implement a per-key guidance state.
...
As the learner becomes stable:

```text
fewer automatic hand animations
```

If a key later becomes weak again:

```text
temporary guidance may return
```

This should feel adaptive, not punitive.

---

# 16. Do not interrupt typing rhythm unnecessarily

The animation is instructional.

It must not become a visual distraction.

During normal active typing:

- do not animate both hands continuously;
- do not chase every typed character;
- do not cover the passage;
- do not stop the round for a single typo;
- do not play loud effects.
...
first introduction
requested hint
repeated-miss recovery
short tutorial
```

During a long AI-generated passage, keep hand guidance minimal unless the learner requests it or repeated errors trigger it.

---

# 17. Reduce Motion and accessibility

Respect the application's Reduce Motion option.
...
```text
home position → target → home
```

with:

```text
target finger highlight
+
...
Do not remove the instructional information.

Keyboard/finger guidance must not depend on color alone.
...
# 18. New-key lesson opening

For a lesson that introduces a new key, use this sequence:
...
"New move: R"
     |
     v
Visual keyboard highlights F + R
     |
     v
Left-index finger animation
F → R → F
     |
     v
Short instruction:
"Left index. Reach up, then come home."
     |
     v
[ Try it ]
     |
     v
3–8 press micro-drill
     |
     v
normal training round
```

Do not make this tutorial long.

The goal is to show the movement and immediately let the learner perform it.

---

# 19. Multi-key lesson visualization

When a stage introduces multiple keys, do not play a long animation movie for all of them at once.
...
Then teach each movement in a short micro-step.

After both are introduced, show alternating practice.

---

# 20. Test/Developer Mode requirements for hand guidance

The existing Test Mode must provide:
...
Allow developers to trigger every finger without changing the learner's actual mastery.

Add a test that loops over the canonical key-to-finger map and verifies every supported typing key resolves to a valid finger/hand instruction.

---

# 21. Required hand-animation automated tests

Add tests for at least:

1. beginner orientation renders both hands;
2. A/S/D/F/J/K/L/; map to correct home fingers;
3. R maps to left index;
4. U maps to right index;
5. new-key introduction plays the proper guide;
6. target finger returns to its home position after guide completes;
7. one normal error does not immediately interrupt with a full animation;
8. repeated misses can trigger guidance;
9. manual `Show finger` always works;
10. guidance frequency decreases after stable mastery;
11. Reduce Motion retains instructional finger/key indication;
12. hand overlay does not capture typing input or block keyboard keys;
13. long Passage Mode does not constantly animate on normal typing;
14. all supported keys resolve through the centralized finger map;
15. Test Mode can trigger hand guides without modifying real learner progress.

---

# 22. Revised account/security tests

In addition to prior auth tests, add:

1. new user can register a passkey;
2. new user cannot enter training until current activation PIN is accepted;
3. correct activation PIN stores the **current activation version**;
4. next passkey login does not ask for unchanged activation PIN;
5. changing the activation PIN increments activation version;
6. old accepted activation version becomes invalid automatically;
7. after a PIN change, passkey login sends learner to the activation screen;
8. entering the new PIN restores access without deleting progress;
9. previous PIN fails after rotation;
10. activation PIN is never stored/logged plaintext;
11. session expires server-side at or before 48 hours;
12. expired session requires passkey authentication again;
13. 48-hour session expiry does not itself invalidate the accepted activation version;
14. explicit logout invalidates session immediately;
15. pending/mismatched activation accounts cannot call MiniMax;
16. changing PIN does not modify learner progress/history;
17. admin PIN and activation PIN are separate authorities.

---

# 23. Revised Definition of Done

REV6 is not complete until all of the following work on the deployed Cloudflare application:

- passkey is the only normal learner login;
- registration creates a discoverable WebAuthn credential;
- WebAuthn verification happens server-side;
- challenges are short-lived and non-replayable;
- activation PIN is checked server-side;
- learner's accepted activation version persists;
- unchanged PIN is **not** requested again after later passkey logins;
- changing the activation PIN increments the activation version;
- learners are prompted for the new PIN after their next passkey login;
- their training data survives the lock/re-activation;
- authenticated sessions have a maximum 48-hour server-side lifetime;
- expired sessions require a passkey again;
- unauthorized/activation-mismatched users cannot access training or MiniMax;
- onboarding occurs only after successful activation;
- onboarding asks name, age, school status, grade conditionally, then character;
- onboarding ends with a short explanation that Cadence teaches touch typing;
- the beginner curriculum visibly demonstrates home-row hand placement;
- Codex has implemented actual finger movement animations;
- new keys animate the correct finger from home position to target and back;
- repeated mistakes can trigger a gentle finger hint;
- manual hand/finger help exists;
- animations become less frequent as mastery improves;
- Reduce Motion has an instructional fallback;
- Test Mode can trigger and validate all finger animations;
- all prior MiniMax, passage mode, dynamic AI options, session save, shortcut, progress, and mastery requirements still work.

---

# 24. Implementation references — use current docs

Codex should verify current package signatures before implementation.
...
Do not replace WebAuthn verification with home-grown cryptography merely because Cloudflare provides Web Crypto. Use the mature WebAuthn library for the ceremony and use Workers crypto primitives only where appropriate for application secrets/hashes/session tokens.

---

# REV7 — Preserve Minimal Training UI; AI/Coach Becomes a Hidden Flyout

> **REV7 overrides any earlier instruction that implies the full AI Training Console should remain permanently visible beside the typing card.**
>
> The default training screen must stay visually close to the current simple Cadence layout: one primary typing card, restrained live metrics, keyboard/finger guidance, and only a very small amount of secondary chrome.
>
> The AI/Coach remains a major feature, but it should be **available on demand rather than constantly occupying screen space**.

## 1. Core UX principle

The typing lesson is the product's primary activity.
...
Do not make the training screen look like:
- a dashboard,
- a chat application,
- an admin console,
- a three-column productivity tool.

Do not add many new boxes merely because the product has more features.

The current visual simplicity should be treated as a design constraint.

## 2. Default training layout

The normal learner view should contain approximately:

```text
Top bar
  Cadence
  small coach mode control
  optional live-metrics toggle
  compact session/menu control

Main area
  centered typing/training card

Inside training card
  module label
  module title
  one short technique instruction
  restrained metrics
  typing text
  progress
  visual keyboard
  contextual finger guidance
  round-complete controls when relevant

AI access
  one compact discoverable control
  hidden flyout by default
```

Do not permanently reserve a wide right-hand column for the chatbot.

The typing card should be able to center naturally in the page when AI is closed.

## 3. Remove the permanent Training Console panel

The current always-visible panel such as:

```text
TRAINING CONSOLE
[ Reshuffle this lesson ]
[ Explain my mistakes ]
[ Practice weak keys ]
```
...
Preferred default label:

```text
✦ Coach
```

because the learner already understands the product context and the word "Coach" is friendlier than "Chatbot".

The control must remain visually subtle but clearly usable.

## 4. Discoverability

The learner should not have to guess that AI exists.

Use at least two discoverability mechanisms:

### A. Visible launcher

Keep a small button in a consistent position, preferably:

```text
top-right portion of the training area
```
...
Do not place it inside the text passage where it competes with typing.
...
Preserve the shortcut from REV4/REV6:

```text
F1 → open AI/Coach flyout
```

When the flyout is open:

```text
F1 → close it and return to training
Esc → close it and return to training
```

On first use or during onboarding, Cadence may briefly teach:

```text
Need help or want a different drill?
Press F1 anytime to open Coach.
```

Show this only briefly.

Do not permanently display instructional text about F1 after the learner has used it.

## 5. Flyout behavior

The Coach should open as a **flyout/drawer**, not a separate full page during normal use.
...
with a practical minimum/maximum based on screen size.

It should overlay or gently push only a small amount of the page.

Do not shrink the main typing card excessively.

For narrow screens:

```text
flyout becomes a near-full-height overlay sheet
```

but still clearly feels temporary.

## 6. Flyout visual treatment

The flyout should match Cadence:

- dark surface,
- subtle border,
- restrained shadows,
- same teal/accent system,
- compact typography,
- no messenger/chat-bubble aesthetic unless useful,
- no giant avatar,
...
Do not prominently advertise the provider/model name to learners.

In admin/test mode only, an optional technical status may show:

```text
MiniMax · connected
```

## 7. Preserve exact training state when opening AI

Opening Coach must never:
...
- complete the lesson,
- lose timing data,
- scroll the long passage back to the top,
- change the target character.

When Coach opens:

```text
pause only the timing metrics that should not count AI discussion time
```

When it closes:

```text
restore typing focus
restore active character
restore passage scroll position
...
## 8. First content shown in the flyout

Do not immediately show an empty generic chat box.

The Coach should open with useful context derived from the current module.
...
You're on Anchor Keys.

Right now you're building F/J home position and clean alternation.
Speed doesn't matter much yet.

What do you want to do?

[ Continue this plan ]
[ Give me different text ]
[ Show finger placement ]
[ Make it easier ]
[ Something else... ]
```

Example for a more advanced learner:

```text
✦ COACH

You're working on sustained accuracy.
...
This preserves the original product principle that the AI should feel like a training tool, not a generic chatbot.

## 9. Progressive option disclosure

The Coach's options should become more sophisticated as the learner progresses.

Do not show every possible AI capability to a new learner.
...
The backend/curriculum controls which actions are valid.

MiniMax must not independently unlock advanced training actions.

## 10. Freeform AI input

Freeform chat remains available, but it is secondary.

At the bottom of the flyout:

```text
Ask Coach anything about your typing...
...
Example accepted requests:

```text
Give me something about running.
Why do I keep missing R?
Give me a longer passage.
Make this less repetitive.
Can I practice punctuation?
```

The AI should respond concisely and then offer 2–4 relevant next actions.

Avoid long essay responses unless the learner explicitly asks for an explanation.

## 11. AI-generated text flow from the flyout
...
Coach should ask only what is necessary.
...
Only show durations/content types allowed by the learner's stage.

After generation:

```text
I made you a 1-minute running passage.
It stays inside the keys you've learned.

[ Start passage ]
[ Try another ]
```

Do not immediately replace an active exercise without confirmation.

## 12. Long passage layout remains separate

When a generated passage is longer than the standard drill:

- close/collapse the Coach flyout after the user presses Start unless they explicitly pin it open;
- widen the typing area;
- reduce text size moderately;
- allow multiline wrapping;
- keep active-line scrolling;
- preserve comfortable line length;
- do not shrink text to fit the entire passage on one screen.

The learner should still be able to press:

```text
F1
```

to reopen the flyout while in Passage Mode.

## 13. AI quick actions without opening full chat

A small number of AI actions can remain accessible outside the flyout **only when contextually useful**.
...
Do not add a row of five or six AI buttons to the main typing card.

Prefer one or two contextual actions maximum.

The rest belong in the flyout.

## 14. Reshuffle shortcut

Preserve:

```text
F2 → reshuffle current/upcoming session
```

The main screen does not need a large permanent Reshuffle button if F2 and the Coach flyout both expose it.

If a visible reshuffle control remains, keep it secondary and contextual.
...
If the current round has unsaved/in-progress typing, preserve the confirmation requirement from earlier revisions.

## 15. Hover/shortcut treatment

Keep shortcut discovery subtle.
...
Reshuffle              F2
Next training round    Shift + Enter
Back to training       Esc
```

Do not permanently print all shortcuts in the main interface.

Use:

- tooltip,
- hover secondary label,
- keyboard-focus helper,
- optional first-use coach tip.

## 16. Coach-mode selector remains small
...
control may remain in the top bar.

It should only control the Coach personality/tone:
...
It must not force the Coach flyout open.

If Coach mode is `Silent`, the learner can still manually open the AI/Coach with F1.

Silent means:

```text
no unsolicited motivational messages
```
...
## 17. Main screen metric restraint

Preserve the current minimal metric treatment.

During typing show only the most useful live metrics:
...
but keep mastery visually quiet.
...
- AI status dashboards,
- long skill lists

on the main typing screen.
...
## 18. Module progress should remain compact

If module mastery is added to the training screen, use something like:
...
Do not add a large card full of completion criteria to the default lesson view.

Detailed criteria can appear on hover, in the session summary, or in Progress.

## 19. Finger guidance and Coach flyout interaction

The hand/finger guidance added in REV6 stays in the typing interface because it is part of actual technique instruction.

However:

- keep hand guidance visually minimal;
- only show it when introducing keys, after repeated errors, or when requested;
- do not permanently show large illustrated hands during every exercise.

Inside Coach, an action:

```text
[ Show finger placement ]
```

should close or partially collapse the flyout and play the relevant finger animation over the visual keyboard.
...
unless the learner deliberately keeps Coach open.

## 20. Flyout open state

Default:

```text
coachFlyoutOpen = false
```

Do not remember "open" across page reloads or new sessions.

Every new training screen should begin with Coach closed.

The learner may optionally open it.

This is important to preserving focus.

## 21. No unsolicited AI popups during active typing

MiniMax/Coach must never automatically open the flyout while the learner is actively typing.

Even if the system detects:
...
respond first with a subtle visual hint.

At a natural break, Cadence may show a tiny passive suggestion such as:

```text
✦ Coach has a suggestion
```

or a small dot on the Coach launcher.

The learner chooses whether to open it.

Never interrupt a typing run with an AI modal.

## 22. Notification indicator

If Coach has a useful observation after a round, use a restrained indicator:
...
No bouncing badge, pulsing animation, red notification dot, or attention-grabbing alert.

Once the learner opens Coach, clear the indicator.

## 23. Mobile/narrow responsive behavior
...
```text
bottom sheet or near-fullscreen overlay
```

Regardless of viewport:

- Close button is obvious.
- Esc works where keyboard exists.
- F1 works on physical keyboard.
- main lesson state survives open/close.
- no accidental page navigation.

## 24. Accessibility

The launcher must be a real accessible button.
...
- opening moves focus to Coach heading/actions/input;
- closing returns focus to the launcher or active typing surface as appropriate.

The flyout must be keyboard navigable.

Do not create a focus trap that prevents Esc from returning to typing.
...
or integrate into the existing app/session store if already clean.

Do not duplicate AI business logic in the UI.

All generation/explanation actions continue through the existing backend APIs.

## 26. Suggested training screen component hierarchy
...
│   ├── RoundProgress
│   ├── VisualKeyboard
│   ├── ContextualFingerGuide
│   └── ContextualRoundActions
│
├── CoachLauncher
│
└── CoachFlyout  // mounted/visible only when requested
```

Do not structure it as:

```text
TrainingCard + PermanentChatColumn + PermanentAnalyticsColumn
```
...
1. training screen loads with Coach flyout closed;
2. main typing card recenters/uses normal layout while Coach is closed;
3. visible Coach launcher exists;
4. launcher tooltip/focus hint shows F1;
5. F1 opens Coach;
6. F1 again closes Coach;
7. Esc closes Coach;
8. opening Coach preserves current typed character index;
9. closing Coach restores typing state/focus;
10. opening Coach pauses only the appropriate session timing;
11. closing Coach resumes timing correctly;
12. active passage scroll position survives Coach open/close;
13. Coach opens with context-aware actions, not an empty generic chat;
14. beginner action set is smaller than advanced action set;
15. freeform AI input remains available;
16. AI-generated passage requires confirmation before replacing active work;
17. starting a long passage collapses Coach by default;
18. Coach never auto-opens because of an error;
19. passive Coach suggestion indicator can appear at a natural break;
20. notification indicator clears after Coach is opened;
21. Silent coach mode does not disable manual F1 access;
22. F2 reshuffle still works with flyout closed;
23. main screen does not show more than the approved compact metric set;
24. flyout is accessible by keyboard and reports `aria-expanded`;
25. narrow-layout flyout does not destroy or reset training state.

## 28. Definition of Done for REV7

REV7 is complete only when:

- the default training screen remains visually simple;
- the permanent right-hand AI panel is removed or hidden by default;
- the learner has an obvious but compact `✦ Coach` launcher;
- F1 opens/closes the Coach;
- Esc closes it;
- AI opens as a flyout/drawer;
- current lesson state is perfectly preserved;
- current typing focus returns correctly;
- the Coach immediately explains the current context and presents useful choices;
- freeform MiniMax chat remains available;
- AI options evolve with curriculum level;
- AI-generated text can start Passage Mode;
- Coach does not auto-open while typing;
- a subtle suggestion indicator can appear only at natural breaks;
- the main typing screen stays focused on typing, keyboard/finger teaching, WPM, accuracy, cadence, and compact mastery;
- no existing passkey, PIN-version activation, 2-day session, MiniMax, save, shortcut, progress, passage, or hand-animation behavior is regressed.

---

# REV8 — Function-Key Coach Choices With Centralized Collision Protection

> **REV8 extends REV7.**
>
> Every structured choice shown by the Coach must be selectable by either mouse/touch or a visible function-key shortcut.
>
> The shortcut system must be centralized so **no function key overlaps with another Cadence action**.

## 1. Global shortcut registry — single source of truth

Do not scatter key assignments across React components.

Create one canonical shortcut registry, for example:
...
export const SHORTCUTS = {
  ...
  reshuffle: "F2",
  nextRound: "Shift+Enter",
  backToTraining: "Escape",

  // Dynamic Coach choice slots
  coachChoice1: "F3",
  coachChoice2: "F4",
  coachChoice3: "F6",
  coachChoice4: "F7",
  ...
  // Explicitly reserved / never claim
  browserRefresh: "F5",
...
All keyboard handlers must import from this registry.

There must not be duplicate hard-coded strings such as `"F3"` spread through the UI.

## 2. Reserved keys and collision rules

### Existing Cadence global functions

These remain permanently reserved:

```text
F1            Open / close Coach
F2            Reshuffle current/upcoming session
Shift+Enter   Next training round
Esc           Back / close Coach / return to training
```

Coach menu options must never use F1 or F2.

### Browser/system keys Cadence must not take over globally

Never assign Coach options to:
...
or other standard browser/window commands.
...
These are active **only while the Coach flyout is open** and only when the corresponding option is visible and enabled.

Because browsers may normally attach behavior to some bare function keys, call `preventDefault()` for these Coach-choice keys **only while the Coach flyout is active and Cadence owns that key in the current state**.

If the browser/platform refuses to intercept a particular key, the visible mouse/touch/button path remains authoritative.

Do not override these keys on public login, passkey, activation-PIN, onboarding, admin, or unrelated screens.

## 3. Every Coach choice shows its key directly

When the Coach presents choices, display the function-key shortcut every time.
...
Right now you're building F/J home position
and clean alternation.

What do you want to do?

[F3] Continue this plan
[F4] Give me different text
[F6] Show finger placement
[F7] Make it easier
[F8] Something else...
```

The keycap should be visible without hover.

Use subdued visual treatment so the option label remains primary.
...
## 4. Dynamic mapping is positional, not semantic

The mapping is temporary and applies to the **current visible option list**.

Do not hard-code:

```text
F3 always = Continue
F4 always = Different text
```

Instead:

```text
F3 = first visible Coach option
F4 = second visible Coach option
F6 = third visible Coach option
F7 = fourth visible Coach option
F8 = fifth visible Coach option
```

This allows options to evolve with learner level.

Example for a beginner:

```text
[F3] Continue
[F4] Show me the finger
[F6] Different drill
[F7] Make it easier
[F8] Something else...
```

Example for a developing learner:

```text
[F3] Continue
[F4] Practice weak keys
[F6] 1-minute passage
[F7] Make it harder
[F8] Pick a topic
```

Example for an advanced learner:

```text
[F3] 3-minute endurance
[F4] Accuracy challenge
[F6] Weak-key drill
[F7] Numbers & symbols
[F8] Pick a topic
```

The backend/curriculum still decides which options are valid.

MiniMax may phrase/recommend choices but must not independently unlock curriculum capabilities.

## 5. Apply function keys to every Coach submenu

This behavior is required every time the Coach asks the learner to choose something.
...
After MiniMax generates a passage:
...
If there are only three choices, F7 and F8 are unbound.

Never keep stale function-key mappings from the previous Coach screen.

## 6. Disabled options

If an option is visible but unavailable:

```text
[F6] Practice weak keys
...
Do not silently route F6 to a different action.

The visible function-key label may remain so the menu layout stays predictable.

## 7. Back behavior and menu hierarchy

Coach behaves like a lightweight game menu.
...
If a Coach submenu is open:

```text
Esc → go back one Coach level
```

If already at Coach root:

```text
Esc → close Coach and return to training
```

F1 always toggles the entire Coach flyout closed/open according to the existing REV7 rules.

## 8. Freeform chat input

The freeform MiniMax input remains available.

While its text field has focus:

- letters, numbers, punctuation and Space remain normal text input;
- Enter sends the request when appropriate;
- Shift+Enter may create a newline only if the input is multiline;
- F3/F4/F6/F7/F8 may activate visible Coach menu options;
- F1 retains Coach-toggle behavior;
- Esc follows Coach back/close behavior;
- F2 reshuffles only if the current context permits reshuffling.

Do not fire Coach function-key actions during an active IME composition event.

## 9. Do not overlap with lesson keystrokes

Function-key Coach choices are active only inside the Coach context.

During the actual typing exercise:

```text
F3/F4/F6/F7/F8 = no Coach choice action
```

unless a future explicitly designed training feature assigns them.

Do not let invisible Coach choices fire while the learner is typing.

This ensures normal training and Coach menus have clean, separate input modes.

## 10. Central collision validator

Add a development-time shortcut collision check.
...
- two global actions with the same key combination;
- a Coach slot that collides with F1/F2;
- any action that claims F5/F11/F12;
- duplicate modified shortcuts;
- a new shortcut that conflicts with `Shift+Enter`;
- inconsistent aliases such as `Esc` vs `Escape` being treated as different keys.

Normalize key names before comparing.
...
A collision should fail tests/build checks rather than silently choosing one action.

## 11. Runtime priority rules

If a key could theoretically apply in more than one context, process it using this priority:

```text
1. Browser/system-reserved keys
2. Authentication/security screens
3. Modal/dialog-specific action
4. Coach flyout navigation/choice
5. Active typing lesson input
6. Global Cadence shortcut
7. No action
```

Exceptions:

- F1 remains the explicit global Coach toggle while inside the authenticated training application.
- F2 remains the explicit global reshuffle shortcut only where reshuffling is valid.
- `Shift+Enter` only advances when Next Round is actually available.

Do not trigger two actions from one keydown event.

Once a handler owns the event:

```text
preventDefault()
stop further Cadence shortcut dispatch
```
...
Global shortcuts remain discoverable without cluttering the main UI:

```text
Coach              F1
Reshuffle          F2
Next round         Shift + Enter
```

Coach menu choice shortcuts are always visible:
...
Each Coach option remains a real accessible button.

Example:

```html
<button aria-keyshortcuts="F3">
  <span class="keycap">F3</span>
  Continue this plan
</button>
```
...
for the currently assigned option key.

Mouse, touch, Tab + Enter/Space, and function key must all invoke the same action.
...
frontend/src/hooks/
  ...
  useCoachShortcuts.ts

frontend/src/utils/
  normalizeShortcut.ts
  validateShortcutRegistry.ts
```

The Coach should provide structured options like:
...
The UI assigns shortcut slots based on visible order.

Do not include literal function keys in MiniMax-generated text.

The frontend/app controller assigns them.

This prevents the AI from inventing or duplicating keyboard mappings.

## 15. Required tests

Add tests for at least:

1. F1 is reserved only for Coach toggle.
2. F2 is reserved only for reshuffle.
3. F5 is never registered by Cadence.
4. F11 and F12 are never registered by Cadence.
5. Coach first option maps to F3.
6. second maps to F4.
7. third maps to F6.
8. fourth maps to F7.
9. fifth maps to F8.
10. Coach options display their keys without hover.
11. clicking an option and pressing its function key call the same handler.
12. changing Coach menu replaces old mappings.
13. invisible options have no active mapping.
14. disabled options do not fire via function key.
15. function keys do not trigger Coach actions while flyout is closed.
16. Esc moves back one Coach submenu before closing root.
17. one keydown cannot trigger two Cadence actions.
18. shortcut registry collision causes a test/build failure.
19. alias normalization catches `Esc`/`Escape` collisions.
20. future shortcuts cannot claim F1/F2/F5/F11/F12 accidentally.
21. freeform input still accepts ordinary typing normally.
22. function-key use does not reset training state.
23. all shortcut-enabled controls expose `aria-keyshortcuts`.
24. browser/public auth screens do not install Coach-option handlers.

## 16. Definition of Done

REV8 is complete only when:

- every structured Coach choice has a visible function-key shortcut;
- mouse/touch and shortcut use the same action code;
- Coach option keys are active only while Coach is open;
- F1 remains Coach toggle;
- F2 remains reshuffle;
- F5/F11/F12 remain untouched;
- no key can trigger two Cadence functions;
- all shortcuts are defined centrally;
- collision validation exists and is tested;
- function-key mappings update cleanly as Coach options change;
- disabled/invisible options cannot fire;
- all previous minimal-UI, MiniMax, passkey, PIN-version, 48-hour session, passage, save, mastery, and finger-animation requirements continue to work.

---

# REV9 — Simplified Learner Access: No Profile PIN, No Admin Gate

> REV9 overrides any earlier requirement that adds a learner/profile PIN, requires admin access for ordinary learner actions, or shows Admin/Test Mode in the normal learner menu.
...
- Coach flyout,
- finger guidance,
- all prior curriculum/progress behavior.

Remove:
- normal learner PIN,
- optional profile PIN,
- admin requirement for player creation,
- admin requirement for Start / Continue,
- Admin / Test Mode from the normal learner menu.

## 1. Final learner flow

```text
Public landing
→ Sign in / create account with passkey
→ If current Cadence access PIN version is not yet accepted:
     show access PIN screen
→ If PIN accepted:
     show CADENCE main menu
→ Start / Continue
```

There is no second learner/profile PIN.

A learner is fully unlocked when:

```text
valid passkey session
AND
accepted_activation_version == current_activation_version
```

Do not add any further learner PIN or admin check.

## 2. Fix current blocked-user behavior

The current issue is that a user can authenticate but then cannot proceed.

Codex must find and remove obsolete gates such as:
...
```text
Cadence access PIN        KEEP
Admin-only legacy code    REMOVE FROM NORMAL LEARNER FLOW
Old learner/profile PIN   REMOVE
```

Do not merely hide a PIN input while leaving a backend or route-guard check in place.

## 3. Access PIN comes before the CADENCE menu
...
Passkey authentication
      |
      v
Current access PIN/version already accepted?
      |
      +---- YES ---> CADENCE MENU
      |
      +---- NO ----> ACCESS PIN SCREEN
                         |
                         +---- correct ---> CADENCE MENU
                         |
                         +---- no PIN ----> Exit safely
```

The full learner menu should not appear before the access gate is satisfied.

## 4. Access PIN screen

Use the same dark/black Cadence visual style.
...
Enter the Cadence access PIN to continue.

[ _ _ _ _   _ _ _ _ ]

[ Unlock ]

Don't have the PIN yet?
Your passkey and account are saved.

[ Exit ]
```

If the PIN changed since last use:

```text
CADENCE

🔒 New access PIN required

Your passkey still works and your progress is safe.
Enter the current Cadence PIN to continue.
...
The sad-face treatment may remain subtle:
...
## 5. Exit without PIN preserves the passkey

If a user has created a passkey but does not have the access PIN:

```text
DO NOT delete account
DO NOT delete passkey
DO NOT restart registration
DO NOT lose saved data
```

`Exit` should:

```text
preserve account/passkey
→ invalidate current session if Exit means sign out
...
After successful passkey authentication and access-PIN approval, show:
...
Remove this normal learner menu item:

```text
Admin / Test mode
```

Do not require admin mode for ordinary testing/use.

## 7. Start / Continue must be unlocked
...
AND
current access PIN/version accepted
```

then:

```text
Start / Continue = enabled
```

Do not condition it on:
- admin state,
- profile PIN,
- optional PIN,
- test mode,
...
```text
Start / Continue
→ load selected/default player
→ restore saved curriculum/session
→ training
```
...
→ Create Player
→ character selection
→ onboarding/hand setup if needed
→ training
```

A new authorized learner must never see the primary action locked.
...
### Keep the characters exactly as they are
...
Only remove the Optional Profile PIN.

## 9. Remove profile-PIN logic completely from learner flow

Remove/disable normal learner use of:

```text
Optional profile PIN
profile PIN prompt
profile PIN hash
requires profile PIN
profile unlocked flag
PIN check on resume
PIN check on character selection
PIN check on Start / Continue
```

If old DB columns exist, first stop reading/writing them.

Do not let legacy `requires_pin = true` values block users.

Drop obsolete columns only in a later safe migration if appropriate.

## 10. Profile creation requires no admin access

A learner may create their own player when:

```text
valid passkey session
AND
accepted current access PIN/version
```
...
Ownership checks still apply so one account cannot access another account's data.

## 11. Remove Admin / Test Mode from normal UX

For now:

```text
do not show Admin / Test mode
```

Do not require an admin PIN for learner testing.

Useful internal developer/test utilities may remain in code if needed, but:
- keep them out of normal learner navigation;
- do not let them block learner routes;
- do not accidentally expose privileged production controls.

## 12. Accessibility and Help stay open to normal users

These must work without admin access:

```text
How training works
Accessibility
Coach
MiniMax training features
Save
Resume
Character selection
```

## 13. Route-guard simplification

Normal learner route guard should be conceptually:
...
if (!acceptedCurrentAccessVersion) {
  return "/activate";
}

return requestedLearnerRoute;
```

Do not add:

```ts
if (!profilePinVerified) ...
if (!adminVerified) ...
```

for learner routes.

## 14. Backend authorization simplification

Normal learner APIs require:

```text
authenticated user
AND
current activation version accepted
AND
requested data belongs to authenticated user
```

No admin claim and no profile-PIN state should be required for:
...
## 15. Onboarding must not reintroduce a learner PIN

First-run flow stays:
...
The passkey already identifies the account.

## 16. Required acceptance flows

### New user with PIN

```text
Create passkey
→ enter Cadence access PIN
→ PIN accepted
→ CADENCE menu
→ Start / Continue enabled
→ Create Player
→ choose Stride/Flux/Vector/Nova
→ onboarding
→ training
```
...
Create passkey
→ access PIN screen
→ user does not know PIN
→ Exit
→ account/passkey remains saved
...
```text
Sign in with passkey
→ accepted activation version matches
→ CADENCE menu
→ Start / Continue enabled
```

### Existing user after access PIN changes
...
No profile/user PIN appears in any of these flows.

## 17. Tests to add/update

Add tests for at least:

1. Create Player form has no Optional Profile PIN.
2. User can create player without admin mode.
3. User can select Stride without admin.
4. User can select Flux without admin.
5. User can select Vector without admin.
6. User can select Nova without admin.
7. Character names/descriptions remain unchanged.
8. Main learner menu has no Admin / Test mode.
9. Access PIN screen appears before main menu when required.
10. Correct access PIN routes to main menu.
11. Start / Continue is enabled immediately after access approval.
12. New authorized user with no player can click Start / Continue.
13. No-player Start / Continue routes to Create Player instead of staying disabled.
14. Existing learner resumes progress.
15. Legacy profile-PIN flags do not block access.
16. Normal learner APIs do not require admin authorization.
17. Normal learner APIs do not require profile-PIN verification.
18. User without access PIN can Exit safely.
19. Exit preserves registered passkey/account.
20. Returning user can use same passkey later.
21. Activation PIN rotation still causes access-version mismatch.
22. After accepting new access PIN, no second/profile PIN is requested.
23. Accessibility works without admin.
24. How training works works without admin.
25. Coach/MiniMax works without admin after normal authorization.
26. Character change preserves progress.
27. Ownership protections still prevent cross-account access.

## 18. Definition of Done

REV9 is complete only when:

- learner logs in with passkey;
- Cadence access PIN appears only before the main menu when required;
- after correct access PIN, learner is fully unlocked;
- Start / Continue is enabled;
- new authorized user can proceed into Create Player/training;
- no normal learner/user PIN exists;
- no Optional Profile PIN exists;
- legacy profile-PIN state cannot lock a user out;
- profile creation does not require admin;
- the four existing characters remain unchanged;
- Admin / Test mode is removed from the normal learner menu;
- normal learner features do not require admin mode;
- Exit from access-PIN screen preserves passkey/account;
- returning users can sign in with that passkey later;
- versioned access PIN and 48-hour session behavior remain intact;
- all prior typing, MiniMax, Coach flyout, save, passage, shortcut, mastery, accessibility, and finger-animation functionality still works.

---

# REV10 — Expand Curriculum to 64 Modules and Add 64 Built-In Texts

> **REV10 ONLY adds curriculum/content.**
>
> Do not change authentication, PIN/passkey behavior, menu flow, Coach shortcuts, character system, UI structure, MiniMax integration, or any other behavior from REV9.
>
> The only product change in this revision is:
>
> - increase the core curriculum to **64 modules**;
> - include **64 built-in texts/drills**, one default built-in text for each module;
> - keep the lesson engine able to generate/reshuffle additional text with MiniMax;
> - use content that feels relevant to modern/Gen-Z learners while also exposing them to classic high-school/American-literature topics.

## 1. Curriculum count is exactly 64

The stable core curriculum must expose:
...
Do not renumber IDs later just because display names change.

Existing saved progress must migrate safely to the new 64-module curriculum.

## 2. Built-in content count is exactly 64

Ship at least:

```text
64 built-in seed texts/drills
```

One is assigned as the default seed for each module.

These are fallback/core texts and must work without MiniMax.

MiniMax can still:
- reshuffle,
- generate variants,
- change topic,
- make passages easier/harder,
- target weak keys,

but the learner must always have a built-in lesson available.

## 3. Content length

Keep default built-in material concise enough for typing practice.

Recommended limits:

```text
Modules 1–15:
very short drills, usually 15–80 characters

Modules 16–31:
1–3 short sentences, usually under 60 words

Modules 32–48:
short passages, usually 30–90 words

Modules 49–64:
short-to-medium passages, usually 35–110 words

Default built-in passage hard cap:
120 words
```

Longer endurance text can still be generated on demand through Passage Mode.

## 4. Copyright rule

Do not paste long copyrighted passages from modern books.

For classic/high-school literature:

- use **original commentary, summaries, theme descriptions, or discussion prompts**;
- public-domain works may use verified public-domain material if desired, but this catalog intentionally uses original prose so there is no dependence on copied source text;
- for still-copyrighted works such as *Of Mice and Men*, *To Kill a Mockingbird*, *The Crucible*, and *Fahrenheit 451*, use only original commentary/summary text, never copied passages.
...
- varied enough that practice does not feel like one long school worksheet.

Use a mix of:
- typing-specific drills,
- school/literature,
- technology,
- gaming,
- running,
...
### Module 01 — Anchor Keys

**Focus:** F/J home anchors

**Built-in text:**

> f j f j   fj jf   f j   jf fj
...
> asdf fdsa   sad fad dad   sass

### Module 03 — Right Home

**Focus:** J K L ;

**Built-in text:**

> jkl; ;lkj   jill   silk   ;lkj

### Module 04 — Home Row Mix

**Focus:** Both home rows

**Built-in text:**

> ask a lad; fall; salad; flask; sad dad

### Module 05 — Center Reach

**Focus:** G / H

**Built-in text:**

> glad had a flag; hall has a dash; hash glass

### Module 06 — Easy Vowels

**Focus:** E / I

**Built-in text:**

> she is safe; he is idle; a fish is at ease
...
> a user is sure; her hair is red; rush; rule

### Module 08 — Wide Reach

**Focus:** W / O

**Built-in text:**

> we row; our world is wide; slow down; glow

### Module 09 — Outer Top Row

**Focus:** Q / P

**Built-in text:**

> people read paper; power up; popular; equip

### Module 10 — Top Row Finish

**Focus:** T / Y

**Built-in text:**

> type your way; try a short route; stay ready

### Module 11 — Lower Center

**Focus:** C / M

**Built-in text:**

> make a calm choice; type more; mix a calm pace

### Module 12 — Lower Reach

**Focus:** V / N

**Built-in text:**

> never give in; move on; every run can win

### Module 13 — Bottom Stretch

**Focus:** X / B

**Built-in text:**

> mix the box; bring extra vibe; big move; next

### Module 14 — Full Alphabet

**Focus:** Z

**Built-in text:**

> zoom past the lazy zone; quiz; jazz; zigzag

### Module 15 — Shift Control

**Focus:** Capital letters

**Built-in text:**

> Start Calm Keep Going Build Rhythm

### Module 16 — Clean Stops

**Focus:** Period / comma

**Built-in text:**

> Start smooth, stay relaxed. Accuracy comes before speed.

### Module 17 — Contractions

**Focus:** Apostrophe

**Built-in text:**

> You're learning the motion. Don't rush; keep your hands relaxed.

### Module 18 — Questions

**Focus:** Question mark / exclamation

**Built-in text:**

> Ready for one more round? Keep it clean, then speed up!

### Module 19 — Quoted Text

**Focus:** Quotation marks

**Built-in text:**

> The coach said, "Find the rhythm, then let the speed follow."

### Module 20 — Number Row I

**Focus:** 1–5

**Built-in text:**

> Run 1 lap, rest 2 minutes, then type 3 clean rounds at 4 or 5 words per burst.

### Module 21 — Number Row II

**Focus:** 6–0

**Built-in text:**

> Try 6 steady lines, 7 calm breaths, 8 clean starts, 9 smooth finishes, and 10 relaxed resets.
...
> A well-timed reset can turn a rough start into a clean, high-focus round.

### Module 23 — Colon + Semicolon

**Focus:** Advanced sentence punctuation

**Built-in text:**

> One rule matters most: stay relaxed; speed is useful only when control stays with it.

### Module 24 — Parentheses

**Focus:** Parenthetical phrases

**Built-in text:**

> A short pause (not a full stop) can help you reset your hands without losing the flow.

### Module 25 — Symbols

**Focus:** Common symbols

**Built-in text:**

> Your goal is simple: 95% accuracy, +1 clean round, and zero panic when a mistake happens.

### Module 26 — Common Words

**Focus:** High-frequency English

**Built-in text:**

> The best typing often feels boring in a good way. Your hands know where to go, so your eyes stay on the screen.

### Module 27 — Word Rhythm

**Focus:** Short word groups

**Built-in text:**

> Keep a steady pace through small words, then let longer words land without forcing them.

### Module 28 — Smooth Sentences

**Focus:** Sentence cadence

**Built-in text:**

> A clean sentence is built one calm keystroke at a time. Fast typing grows from repeatable motion.

### Module 29 — Contraction Flow

**Focus:** Natural writing

**Built-in text:**

> It's easier to stay smooth when you're not fighting the keyboard. Let your fingers move, then return home.

### Module 30 — Capital Rhythm

**Focus:** Shift without stalling

**Built-in text:**

> Monday starts fast, Tuesday feels normal, and Friday arrives before anyone expects it.

### Module 31 — Punctuation Mix

**Focus:** Mixed punctuation

**Built-in text:**

> Focus first, speed second: if the rhythm breaks, reset, breathe, and continue without making the mistake a big deal.

### Module 32 — One-Minute Base

**Focus:** First sustained passage

**Built-in text:**

> A minute of steady typing tells you more than a five-second burst. Keep your shoulders loose, watch the screen, and aim for a pace you could repeat again.

### Module 33 — Poe: Suspense

**Focus:** American literature — original commentary

**Built-in text:**

> Edgar Allan Poe builds suspense by narrowing attention. A sound, a shadow, or a single thought can become huge when a character cannot stop thinking about it.

### Module 34 — Poe: The Raven

**Focus:** American literature — original commentary

**Built-in text:**

> In The Raven, repetition gives the poem a pulse. The speaker keeps returning to the same question, and the repeated answer makes the room feel smaller and darker each time.

### Module 35 — The Scarlet Letter

**Focus:** American literature — original commentary

**Built-in text:**

> The Scarlet Letter asks what happens when a community turns one mistake into a permanent identity. The story keeps returning to guilt, reputation, and the difference between public judgment and private truth.
...
> Moby-Dick is more than a chase at sea. Captain Ahab turns one goal into an obsession, and the crew has to live inside the consequences of a leader who cannot let go.

### Module 37 — Huckleberry Finn

**Focus:** American literature — original commentary

**Built-in text:**

> Huckleberry Finn follows a boy who keeps noticing that the rules adults defend do not always match what feels morally right. The journey becomes a test of judgment as much as adventure.

### Module 38 — The Great Gatsby

**Focus:** American literature — original commentary

**Built-in text:**

> The Great Gatsby explores ambition, image, and the idea that a person can rebuild the past if they try hard enough. The glitter is obvious; the cost is easier to miss.

### Module 39 — The Red Badge of Courage

**Focus:** American literature — original commentary

**Built-in text:**

> The Red Badge of Courage focuses on fear before heroism. The young soldier wants to know what he will do under pressure, but the answer only appears when the situation becomes real.

### Module 40 — The Call of the Wild

**Focus:** American literature — original commentary

**Built-in text:**

> The Call of the Wild follows a dog forced to adapt to a harsher world. Strength matters, but attention, instinct, and the ability to learn quickly matter just as much.

### Module 41 — The Awakening

**Focus:** American literature — original commentary

**Built-in text:**

> The Awakening follows a woman questioning the life that other people have planned for her. The novel is interested in independence, expectation, and the cost of wanting a different path.

### Module 42 — Leaves of Grass

**Focus:** American literature — original commentary

**Built-in text:**

> Walt Whitman's poetry often treats ordinary life as something worth noticing. Crowds, work, movement, and the body all become part of a bigger idea about identity and connection.

### Module 43 — Emily Dickinson

**Focus:** American literature — original commentary

**Built-in text:**

> Emily Dickinson can turn a small image into a large question. A room, a bird, a pause, or a moment of silence can suddenly become a way to think about time, fear, hope, or death.

### Module 44 — Frederick Douglass

**Focus:** American literature — original commentary

**Built-in text:**

> Frederick Douglass writes about literacy as power. Learning to read did not simply give him information; it changed what he could understand about freedom, control, and the world around him.

### Module 45 — The Crucible

**Focus:** High-school literature — original commentary only

**Built-in text:**

> The Crucible shows how fear can spread through a community faster than evidence. Once reputation and accusation become weapons, people start making choices based on survival instead of truth.

### Module 46 — Of Mice and Men

**Focus:** High-school literature — original commentary only

**Built-in text:**

> Of Mice and Men centers on friendship, work, and fragile plans for a better future. George and Lennie keep returning to the same dream because imagining it gives their difficult life a direction.

### Module 47 — To Kill a Mockingbird

**Focus:** High-school literature — original commentary only

**Built-in text:**

> To Kill a Mockingbird looks at justice through a child's changing perspective. As Scout learns more about the adults around her, simple ideas about fairness become more complicated.

### Module 48 — Fahrenheit 451

**Focus:** High-school literature — original commentary only

**Built-in text:**

> Fahrenheit 451 imagines a society where distraction is easier than reflection. The story asks what happens when people stop choosing difficult ideas because quick entertainment feels more comfortable.

### Module 49 — Cross-Country Morning

**Focus:** Gen Z / running

**Built-in text:**

> The first mile can feel awkward even when you're fit. Your breathing is still settling, your legs are waking up, and the goal is not to win the warm-up. The goal is to find a pace you can own.

### Module 50 — Game Patch Notes

**Focus:** Gen Z / gaming

**Built-in text:**

> A good game update changes just enough to make players rethink old habits. One weapon gets weaker, another gets useful, and suddenly everyone has to test what actually works instead of copying yesterday's strategy.

### Module 51 — Group Chat Timing

**Focus:** Gen Z / social

**Built-in text:**

> A group chat moves fast until someone sends a serious question. Then the typing bubbles appear, disappear, and appear again. Sometimes the most useful reply is the one that took an extra minute to think through.

### Module 52 — Algorithm Feed

**Focus:** Gen Z / internet culture

**Built-in text:**

> An algorithm learns what keeps your attention, not necessarily what improves your day. If every swipe gives it more data, choosing when to stop becomes part of using the app instead of being used by it.

### Module 53 — Music Discovery

**Focus:** Gen Z / music

**Built-in text:**

> Finding a new song can feel random, but recommendations usually follow patterns. One track leads to another, a playlist shifts your taste, and a song you skipped last month can suddenly make sense.

### Module 54 — Coding Bug

**Focus:** Gen Z / technology

**Built-in text:**

> The bug looked impossible until one line changed. The program was not broken everywhere; one assumption was wrong near the start, and every later result faithfully repeated that mistake.

### Module 55 — AI as a Tool

**Focus:** Gen Z / technology

**Built-in text:**

> AI is useful when the goal is clear. Ask it to explain, compare, generate a practice set, or challenge an idea. The better the task is defined, the easier it is to judge whether the answer actually helps.

### Module 56 — Passkeys

**Focus:** Gen Z / security

**Built-in text:**

> A passkey replaces a password with a cryptographic credential stored on a trusted device or security key. The site verifies that credential without needing you to memorize another secret phrase.

### Module 57 — Space Signal

**Focus:** Science / fiction

**Built-in text:**

> The signal appeared for eleven seconds, vanished, and returned at the same interval. Nobody called it alien. Not yet. The interesting part was simpler: something out there was repeating on purpose.

### Module 58 — Climate Data

**Focus:** Science

**Built-in text:**

> One unusually hot day does not prove a climate trend, just as one cold day does not disprove one. Scientists look at patterns across years, regions, oceans, ice, air, and many different measurements.

### Module 59 — Study Sprint

**Focus:** School / productivity

**Built-in text:**

> A focused twenty-minute study sprint can beat an hour of switching between tabs. Pick one task, remove the easy distractions, and finish a clear chunk before deciding what comes next.

### Module 60 — Debate Prep

**Focus:** School / reasoning

**Built-in text:**

> A strong argument does more than repeat a claim. It gives evidence, explains why that evidence matters, and takes the best opposing point seriously enough to answer it.

### Module 61 — Short Story: Last Bus

**Focus:** Original fiction

**Built-in text:**

> The bus doors closed just as Maya reached the corner. Instead of chasing it, she stopped. Across the street, a small bookstore still had its lights on, and a handwritten sign promised free hot chocolate after eight.

### Module 62 — Short Story: Save Point

**Focus:** Original fiction

**Built-in text:**

> Leo had restarted the level six times, but the seventh run felt different. He stopped rushing the opening section, saved his best move for the hard part, and reached the final room with more health than ever.

### Module 63 — Endurance Passage

**Focus:** Long-form control

**Built-in text:**

> Speed is exciting because it is easy to notice, but endurance is quieter. Real fluency means holding a useful pace while sentences get longer, punctuation changes, and attention starts to drift. The goal is not a perfect line. The goal is control that lasts.

### Module 64 — Personal Best

**Focus:** Final mixed challenge

**Built-in text:**

> A personal best is not proof that every future session will be faster. It is proof that the skill exists. Good training makes that performance easier to repeat, even on an ordinary day when the keyboard feels less automatic.


## 7. Curriculum progression

Treat the 64 modules as four broad phases:

```text
PHASE 1 — Foundations
Modules 01–16
Home row, reach mechanics, full alphabet, Shift, comma/period

PHASE 2 — Fluency Tools
Modules 17–32
Contractions, punctuation, numbers, symbols, sentence rhythm,
mixed punctuation, first sustained one-minute passage

PHASE 3 — Reading + American Literature
Modules 33–48
Short original commentary about classic American/high-school literature

PHASE 4 — Modern Fluency
Modules 49–64
Running, gaming, internet culture, music, coding, AI, passkeys,
science, school reasoning, original fiction, endurance, personal best
```

## 8. Built-in data structure

Store the built-in modules as structured data, not hard-coded inside React components.
...
type CurriculumModule = {
  ...
  order: number;
  title: string;
  phase: "foundation" | "fluency" | "literature" | "modern";
  focus: string;
  allowedKeys: string[];
  introducedKeys: string[];
  builtInTextId: string;
...
```text
frontend/src/curriculum/modules.ts
frontend/src/content/builtinTexts.ts
```

or use the equivalent existing repository structure.

## 9. Early-module key validation

For Modules 01–14, validate that the built-in text does not use letters that have not yet been introduced.

Codex must write automated validation that checks each restricted-key lesson.

If a text includes an unavailable key:
...
For punctuation modules, include the newly introduced punctuation only after its module unlocks it.

## 10. Literature metadata

For Modules 33–48, tag each text:
...
Add the work/author as metadata, for example:

```ts
literatureWork: "The Great Gatsby"
literatureAuthor: "F. Scott Fitzgerald"
```

Do not imply that the built-in text is a quotation from the book.

If displayed in the UI, use:

```text
Inspired by / About: The Great Gatsby
```
...
unless an actual verified excerpt is intentionally added later.

## 11. AI use with these modules

MiniMax may generate a fresh version of a module while preserving:
...
a new original Gatsby-themed typing passage

MiniMax may not:
paste a copyrighted passage,
introduce unsupported characters,
unlock a later curriculum skill.
```

## 12. User topic choices

Once the learner is advanced enough for unrestricted text, the Coach may offer topic shortcuts such as:
...
The 64 built-in texts remain available even if AI is offline.

## 13. Migration

If the current app has fewer modules:

- preserve existing learner progress;
- map existing stages/modules to the closest corresponding new module IDs;
- do not reset mastery;
- do not make completed skills disappear;
- add safe defaults for newly introduced modules.
...
Increment it for this 64-module expansion.

## 14. Tests

Add tests that verify:

1. curriculum contains exactly 64 modules;
2. module IDs are unique;
3. module order is 1–64 with no gaps;
4. built-in text catalog contains at least 64 texts;
5. every module has a valid built-in text;
6. every built-in text points to a real module;
7. Modules 01–14 obey allowed-key restrictions;
8. literature texts are marked `original_commentary`;
9. no literature entry is labeled as an excerpt unless explicitly verified;
10. default passage length is <= 120 words;
11. offline mode can start every one of the 64 modules;
12. MiniMax reshuffle preserves each module's curriculum constraints;
13. existing progress migrates without reset.

## 15. Definition of Done

REV10 is complete when:

- the curriculum contains exactly 64 stable modules;
- the app ships with 64 built-in default texts/drills;
- all 64 work without AI;
- early restricted-key lessons are validated automatically;
- the content mix includes modern Gen-Z-friendly topics;
- the content mix includes classic American/high-school literature through original commentary;
- no still-copyrighted book text is copied into the built-in catalog;
- MiniMax can create fresh variants without changing curriculum rules;
- existing saved progress survives the curriculum expansion;
- no non-content behavior from REV9 is changed.
