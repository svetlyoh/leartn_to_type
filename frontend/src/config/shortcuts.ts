...
export function isHandsToggleChord(event:Pick<KeyboardEvent,'key'|'shiftKey'|'ctrlKey'|'metaKey'|'altKey'>,pressedCodes:ReadonlySet<string>):boolean{
 return event.key==='{'&&event.shiftKey&&!event.ctrlKey&&!event.metaKey&&!event.altKey&&pressedCodes.has('Space');
}
